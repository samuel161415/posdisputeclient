...
function Declined() {
    ...
    useEffect(()=>{
       const makeRequest = async () => {
           ...
           try {
            ...
            const res = context.currentBranch==='AR_TEAM' ? await userRequest.get('/declined/get'): await userRequest.get(`/declined/get/${context.currentBranch}`) ;
            const keys = []
            res.data.map(item =>(
            keys.push(item.key)
            ))
    ...
    const openModal = async(key) => {
       ...
       const sres = await userRequest.get(`/declined/single/${key}`)
      ...
      {login?(<>
        <div class = 'flex justify-between'>
            ...
            <div class = 'flex justify-end mb-3'>
                <ExportToExcel tableData={data} filename={'Declined'}/>
       ...
       {/* <div class='flex flex-row justify-evenly mb-2 border-b-2'>
           ...
           <Link to={'/declined'}><button class='p-2 rounded-t border-x-2 border-t-2 shadow-md' onClick={(e)=>{context.setColor(4)}}>Declined</button></Link>
       ...
       <Declined_modal isOpen={isModalOpen} onClose={closeModal}  data={selectedData}  singleData= {singleData}/>
      </>):
      (<Navigate to={'/login'} />)}
   </div>
 )
